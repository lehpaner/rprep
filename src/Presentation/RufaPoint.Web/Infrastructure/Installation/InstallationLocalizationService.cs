...
namespace RufaPoint.Web.Infrastructure.Installation
{
    /// <summary>
    ...
    public partial class InstallationLocalizationService : IInstallationLocalizationService
    {
        ...
        /// </summary>
        private const string LANGUAGE_COOKIE_NAME = ".RufaPoint.Installation.Lang";
            ...
            foreach (var filePath in Directory.EnumerateFiles(CommonHelper.MapPath("~/App_Data/Localization/Installation/"), "*.xml"))
            {
                ...
                foreach (XmlNode resNode in resources)
                {
                    ...
                    if (resNameAttribute == null)
                        throw new CoreException("All installation resources must have an attribute Name=\"Value\".");
                    var resourceName = resNameAttribute.Value.Trim();
                    if (string.IsNullOrEmpty(resourceName))
                        throw new CoreException("All installation resource attributes 'Name' must have a value.'");

                    if (resValueNode == null)
                        throw new CoreException("All installation resources must have an element \"Value\".");
