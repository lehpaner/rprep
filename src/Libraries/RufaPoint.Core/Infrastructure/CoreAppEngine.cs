...
namespace RufaPoint.Core.Infrastructure
{
    ...
    class CoreAppEngine : IEngine
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services, IConfigurationRoot configuration)
        {
            ...
            var startupConfigurations = typeFinder.FindClassesOfType<ICoreAppStartup>();

            //create and sort instances of startup configurations
            var instances = startupConfigurations
                .Where(startup => PluginManager.FindPlugin(startup)?.Installed ?? true) //ignore not installed plugins
                .Select(startup => (ICoreAppStartup)Activator.CreateInstance(startup))
            ...
            var nopConfig = services.BuildServiceProvider().GetService<CoreAppConfig>();
